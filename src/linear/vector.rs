...
impl PartialEq for Vec2 {
	...
	fn eq( &self, rhs: &Self ) -> bool {
		if (*self - *rhs).x.abs() < 0.00001  && (*self - *rhs).y.abs() < 0.00001 && self.coord_system == rhs.coord_system {
			true
		} else {
			false
		}
...
impl Default for Vec3 {
    fn default() -> Self {
        Self { x: 0.0, y: 0.0, z: 0.0, coord_system: CARTESIAN }
    }
}
...
impl Vec3 {
	...
	///
	pub fn new( ) -> Vec3 {
		Vec3 { x: 0.0, y: 0.0, z: 0.0, coord_system: CARTESIAN }
	}
	...
	pub fn from_angle( theta: &f32, phi: &f32, in_mag: &Option<f32> ) -> Vec3 {
		if let Some(mag) = in_mag {
			let mut temp = Vec3 { x: *mag, y: *theta, z: *phi, coord_system: POLAR };
			temp.swap_system( CARTESIAN );
			temp
		} else {
			let mut temp = Vec3 { x: 1.0, y: *theta, z: *phi, coord_system: POLAR };
			temp.swap_system( CARTESIAN );
			temp
		}
	}
	...
	pub fn from_rand_angle( range: &Range<f32>, mag: &Option<f32> ) -> Vec3 {
		let temp = Vec3::from_angle( &rand::thread_rng().gen_range( range.clone() ), 
	...
	pub fn cross( &mut self, rhs: &Vec3 ) -> Vec3 {
		...
		Vec3 { x, y, z, coord_system: CARTESIAN }
	}
